...
typedef LIBNIFTI_EXPORT struct {                   /** 4x4 matrix struct **/
  float m[4][4] ;
} mat44 ;

typedef LIBNIFTI_EXPORT struct {                   /** 3x3 matrix struct **/
  float m[3][3] ;
} mat33 ;
 ...
 *         codes.
 */
typedef LIBNIFTI_EXPORT enum _analyze75_orient_code {
 ...
 */
typedef LIBNIFTI_EXPORT struct {                /*!< Image storage struct **/
...
typedef LIBNIFTI_EXPORT struct {
...
LIBNIFTI_EXPORT char *nifti_datatype_string   ( int dt ) ;
LIBNIFTI_EXPORT char *nifti_units_string      ( int uu ) ;
LIBNIFTI_EXPORT char *nifti_intent_string     ( int ii ) ;
LIBNIFTI_EXPORT char *nifti_xform_string      ( int xx ) ;
LIBNIFTI_EXPORT char *nifti_slice_string      ( int ss ) ;
LIBNIFTI_EXPORT char *nifti_orientation_string( int ii ) ;

LIBNIFTI_EXPORT int   nifti_is_inttype( int dt ) ;

LIBNIFTI_EXPORT mat44 nifti_mat44_inverse( mat44 R ) ;

LIBNIFTI_EXPORT mat33 nifti_mat33_inverse( mat33 R ) ;
LIBNIFTI_EXPORT mat33 nifti_mat33_polar  ( mat33 A ) ;
LIBNIFTI_EXPORT float nifti_mat33_rownorm( mat33 A ) ;
LIBNIFTI_EXPORT float nifti_mat33_colnorm( mat33 A ) ;
LIBNIFTI_EXPORT float nifti_mat33_determ ( mat33 R ) ;
LIBNIFTI_EXPORT mat33 nifti_mat33_mul    ( mat33 A , mat33 B ) ;

LIBNIFTI_EXPORT void nifti_swap_2bytes ( size_t n , void *ar ) ;
LIBNIFTI_EXPORT void nifti_swap_4bytes ( size_t n , void *ar ) ;
LIBNIFTI_EXPORT void nifti_swap_8bytes ( size_t n , void *ar ) ;
LIBNIFTI_EXPORT void nifti_swap_16bytes( size_t n , void *ar ) ;
LIBNIFTI_EXPORT void nifti_swap_Nbytes ( size_t n , int siz , void *ar ) ;

LIBNIFTI_EXPORT int    nifti_datatype_is_valid   (int dtype, int for_nifti);
LIBNIFTI_EXPORT int    nifti_datatype_from_string(const char * name);
LIBNIFTI_EXPORT char * nifti_datatype_to_string  (int dtype);

LIBNIFTI_EXPORT int   nifti_get_filesize( const char *pathname ) ;
LIBNIFTI_EXPORT void  swap_nifti_header ( struct nifti_1_header *h , int is_nifti ) ;
LIBNIFTI_EXPORT void  old_swap_nifti_header( struct nifti_1_header *h , int is_nifti );
LIBNIFTI_EXPORT int   nifti_swap_as_analyze( nifti_analyze75 *h );


/* main read/write routines */

LIBNIFTI_EXPORT nifti_image *nifti_image_read_bricks(const char *hname , int nbricks,
                                     const int *blist, nifti_brick_list * NBL);
LIBNIFTI_EXPORT int          nifti_image_load_bricks(nifti_image *nim , int nbricks,
                                     const int *blist, nifti_brick_list * NBL);
LIBNIFTI_EXPORT void         nifti_free_NBL( nifti_brick_list * NBL );

LIBNIFTI_EXPORT nifti_image *nifti_image_read    ( const char *hname , int read_data ) ;
LIBNIFTI_EXPORT int          nifti_image_load    ( nifti_image *nim ) ;
LIBNIFTI_EXPORT void         nifti_image_unload  ( nifti_image *nim ) ;
LIBNIFTI_EXPORT void         nifti_image_free    ( nifti_image *nim ) ;

LIBNIFTI_EXPORT int          nifti_read_collapsed_image( nifti_image * nim, const int dims [8],
                                         void ** data );

LIBNIFTI_EXPORT int          nifti_read_subregion_image( nifti_image * nim,
                                         int *start_index, int *region_size,
                                         void ** data );

LIBNIFTI_EXPORT void         nifti_image_write   ( nifti_image * nim ) ;
LIBNIFTI_EXPORT void         nifti_image_write_bricks(nifti_image * nim,
                                      const nifti_brick_list * NBL);
LIBNIFTI_EXPORT void         nifti_image_infodump( const nifti_image * nim ) ;

LIBNIFTI_EXPORT void         nifti_disp_lib_hist( void ) ;     /* to display library history */
LIBNIFTI_EXPORT void         nifti_disp_lib_version( void ) ;  /* to display library version */
LIBNIFTI_EXPORT int          nifti_disp_matrix_orient( const char * mesg, mat44 mat );
LIBNIFTI_EXPORT int          nifti_disp_type_list( int which );


LIBNIFTI_EXPORT char *       nifti_image_to_ascii  ( const nifti_image * nim ) ;
LIBNIFTI_EXPORT nifti_image *nifti_image_from_ascii( const char * str, int * bytes_read ) ;

LIBNIFTI_EXPORT size_t       nifti_get_volsize(const nifti_image *nim) ;

/* basic file operations */
LIBNIFTI_EXPORT int    nifti_set_filenames(nifti_image * nim, const char * prefix, int check,
                           int set_byte_order);
LIBNIFTI_EXPORT char * nifti_makehdrname  (const char * prefix, int nifti_type, int check,
                           int comp);
LIBNIFTI_EXPORT char * nifti_makeimgname  (const char * prefix, int nifti_type, int check,
                           int comp);
LIBNIFTI_EXPORT int    is_nifti_file      (const char *hname);
LIBNIFTI_EXPORT char * nifti_find_file_extension(const char * name);
LIBNIFTI_EXPORT int    nifti_is_complete_filename(const char* fname);
LIBNIFTI_EXPORT int    nifti_validfilename(const char* fname);

LIBNIFTI_EXPORT int    disp_nifti_1_header(const char * info, const nifti_1_header * hp ) ;
LIBNIFTI_EXPORT void   nifti_set_debug_level( int level ) ;
LIBNIFTI_EXPORT void   nifti_set_skip_blank_ext( int skip ) ;
LIBNIFTI_EXPORT void   nifti_set_allow_upper_fext( int allow ) ;

LIBNIFTI_EXPORT int    valid_nifti_brick_list(nifti_image * nim , int nbricks,
                              const int * blist, int disp_error);

/* znzFile operations */
LIBNIFTI_EXPORT znzFile nifti_image_open(const char * hname, char * opts, nifti_image ** nim);
LIBNIFTI_EXPORT znzFile nifti_image_write_hdr_img(nifti_image *nim, int write_data,
                                  const char* opts);
LIBNIFTI_EXPORT znzFile nifti_image_write_hdr_img2( nifti_image *nim , int write_opts ,
               const char* opts, znzFile imgfile, const nifti_brick_list * NBL);
LIBNIFTI_EXPORT size_t  nifti_read_buffer(znzFile fp, void* datatptr, size_t ntot,
                         nifti_image *nim);
LIBNIFTI_EXPORT int     nifti_write_all_data(znzFile fp, nifti_image * nim,
                             const nifti_brick_list * NBL);
LIBNIFTI_EXPORT size_t  nifti_write_buffer(znzFile fp, const void * buffer, size_t numbytes);
LIBNIFTI_EXPORT nifti_image *nifti_read_ascii_image(znzFile fp, char *fname, int flen,
                         int read_data);
LIBNIFTI_EXPORT znzFile nifti_write_ascii_image(nifti_image *nim, const nifti_brick_list * NBL,
                         const char * opts, int write_data, int leave_open);


LIBNIFTI_EXPORT void nifti_datatype_sizes( int datatype , int *nbyper, int *swapsize ) ;

LIBNIFTI_EXPORT void nifti_mat44_to_quatern( mat44 R ,
                             float *qb, float *qc, float *qd,
                             float *qx, float *qy, float *qz,
                             float *dx, float *dy, float *dz, float *qfac ) ;

LIBNIFTI_EXPORT mat44 nifti_quatern_to_mat44( float qb, float qc, float qd,
                              float qx, float qy, float qz,
                              float dx, float dy, float dz, float qfac );

LIBNIFTI_EXPORT mat44 nifti_make_orthog_mat44( float r11, float r12, float r13 ,
                               float r21, float r22, float r23 ,
                               float r31, float r32, float r33  ) ;

LIBNIFTI_EXPORT int nifti_short_order(void) ;              /* CPU byte order */
...
LIBNIFTI_EXPORT void nifti_mat44_to_orientation( mat44 R , int *icod, int *jcod, int *kcod ) ;

/*--------------------- Low level IO routines ------------------------------*/

LIBNIFTI_EXPORT char * nifti_findhdrname (const char* fname);
LIBNIFTI_EXPORT char * nifti_findimgname (const char* fname , int nifti_type);
LIBNIFTI_EXPORT int    nifti_is_gzfile   (const char* fname);

LIBNIFTI_EXPORT char * nifti_makebasename(const char* fname);


/* other routines */
LIBNIFTI_EXPORT struct nifti_1_header   nifti_convert_nim2nhdr(const nifti_image* nim);
LIBNIFTI_EXPORT nifti_1_header * nifti_make_new_header(const int arg_dims[], int arg_dtype);
LIBNIFTI_EXPORT nifti_1_header * nifti_read_header(const char *hname, int *swapped, int check);
LIBNIFTI_EXPORT nifti_image    * nifti_copy_nim_info(const nifti_image * src);
LIBNIFTI_EXPORT nifti_image    * nifti_make_new_nim(const int dims[], int datatype,
                                                      int data_fill);
LIBNIFTI_EXPORT nifti_image    * nifti_simple_init_nim(void);
LIBNIFTI_EXPORT nifti_image    * nifti_convert_nhdr2nim(struct nifti_1_header nhdr,
                                        const char * fname);

LIBNIFTI_EXPORT int    nifti_hdr_looks_good        (const nifti_1_header * hdr);
LIBNIFTI_EXPORT int    nifti_is_valid_datatype     (int dtype);
LIBNIFTI_EXPORT int    nifti_is_valid_ecode        (int ecode);
LIBNIFTI_EXPORT int    nifti_nim_is_valid          (nifti_image * nim, int complain);
LIBNIFTI_EXPORT int    nifti_nim_has_valid_dims    (nifti_image * nim, int complain);
LIBNIFTI_EXPORT int    is_valid_nifti_type         (int nifti_type);
LIBNIFTI_EXPORT int    nifti_test_datatype_sizes   (int verb);
LIBNIFTI_EXPORT int    nifti_type_and_names_match  (nifti_image * nim, int show_warn);
LIBNIFTI_EXPORT int    nifti_update_dims_from_array(nifti_image * nim);
LIBNIFTI_EXPORT void   nifti_set_iname_offset      (nifti_image *nim);
LIBNIFTI_EXPORT int    nifti_set_type_from_names   (nifti_image * nim);
LIBNIFTI_EXPORT int    nifti_add_extension(nifti_image * nim, const char * data, int len,
                           int ecode );
LIBNIFTI_EXPORT int    nifti_compiled_with_zlib    (void);
LIBNIFTI_EXPORT int    nifti_copy_extensions (nifti_image *nim_dest,const nifti_image *nim_src);
LIBNIFTI_EXPORT int    nifti_free_extensions (nifti_image *nim);
LIBNIFTI_EXPORT int  * nifti_get_intlist     (int nvals , const char *str);
LIBNIFTI_EXPORT char * nifti_strdup          (const char *str);
LIBNIFTI_EXPORT int    valid_nifti_extensions(const nifti_image *nim);
